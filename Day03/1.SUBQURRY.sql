...
-- 서브쿼리를 사용해서 출판사이름이 '미디어' 끝나는 출판사에서
-- 출판한 책을 조회하시오
SELECT *
  FROM Book b
...
-- 출판사별 가장비싼 책과 가격이 일치하는 책정보 조회하라
 SELECT *
   FROM Book b
...
-- ALL = AND 
-- 출판사별 가장비싼 책들과 가격이 모두 조건에 일치하는 책정보 조회하라
SELECT *
   FROM Book b
...
-- 상관서브쿼리(Corrlated subquery). 
-- 메인쿼리의 컬럼이 서브쿼리의 조건에 포함될 떄
-- 서브쿼리만 따로 실행 불가능
...
-- 서브쿼리로 만들어진 가상테이블을 진짜 테이블처럼 FROM 절에 사용하는 것
-- 구매 고객별 합계 중에서 총 금액이 3만 5천 이상인 구매건의 고객번호와 금액을 조회하세요
...
-- From절 서브쿼리 일반 테이블(Customer과 조인 가능
...
/*SELECT절 서브쿼리*/
-- 조인을 사용하지 않고 주문번호 5 이하인 주문건들의 고객명과 책이름을 같이 조회하시오
-- 조인으로 했을떄 쿼리
SELECT o.orderid 
     , o.custid 
     , o.bookid 
